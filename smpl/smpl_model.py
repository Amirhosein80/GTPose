...
class ParametricModel:
    ...
    def get_bone_features_and_adj_matrix(self, shape: torch.Tensor = None):
        r"""
        Generate bone length and adjacy matrix for bone connetions. (torch, batch)

        
        :param shape: Tensor for model shapes that can expand to [batch_size, 10]. Use None for the mean(zero) shape.
        :return: Bone length in [num_joint, num_joint],
                 Bone connections in [num_joint, num_joint].
        """
        j, _ = self.get_zero_pose_joint_and_vertex(shape)
        bl = self.joint_position_to_bone_vector(j.unsqueeze(0)).norm(dim=-1, keepdim=True)[0]
        blm = torch.zeros(bl.shape[-2], bl.shape[-2])
        adj_matirx = torch.zeros_like(blm)
        for idx, (i, j) in enumerate(self.bone_connetcion.T[1:]):
            blm[i, j] = bl[idx]
            blm[j, i] = bl[idx]
            adj_matirx[i, j] = 1.0
            adj_matirx[j, i] = 1.0
        blm = blm.to(self.device)
        adj_matirx = adj_matirx.to(self.device)
        return blm, adj_matirx
